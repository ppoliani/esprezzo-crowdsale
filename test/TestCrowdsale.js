...
const {calculateTokens} = require('./helpers')
...
contract('EsprezzoCrowdsale', async (accounts) => {
  ...
  it('one ETH should buy 6257 Esprezzo Tokens in PreICO', async () => {
    ...
    assert.equal(tokenAmount.toNumber(), calculateTokens(1), 'The sender didn\'t receive the tokens as per PreICO rate');
  });
  ...
  it('one ETH should buy 2 Esprezzo Tokens in ICO', async () => {
    ...
    assert.equal(tokenAmount.toNumber(), calculateTokens(1.5), 'The sender didn\'t receive the tokens as per ICO rate');
  });
